...
function show_form() {
    ...
    if(isset($_POST["dalid"])) {
        $result = $database->query("update renginiu_dalyviai set patvirtinimas = 0 where naudotojo_id = ".$naudotojoID['id']." and renginio_id = ".$_POST["rngid"]);
        echo 'Registracija sėkminga <a href="renginys.php?rngid='.$_POST["rngid"].'">Atgal</a>';
        $_POST = array();
    } else {
        if(isset($_GET["dalid"])) {
            $result = $database->query("select rd.papildoma_info, n.vardas, n.pavarde, rd.renginio_id from renginiu_dalyviai rd, naudotojas n where rd.naudotojo_id = n.id and rd.naudotojo_id = ".$naudotojoID['id']." and rd.patvirtinimas = 1");
            if($dalyvis = $result->fetch_assoc()) {
                ...
                echo '<button type="submit" class="btn btn-default">Atšaukti registraciją</button></form>';
            } else {
                echo 'Dalyvio negalima atšaukti';
            }
        } else {
            echo 'Klaida!';
        }
